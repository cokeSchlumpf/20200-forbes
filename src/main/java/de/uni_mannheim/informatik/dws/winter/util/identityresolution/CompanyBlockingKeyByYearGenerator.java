 ...
 * 
 */
public class CompanyBlockingKeyByYearGenerator extends RecordBlockingKeyGenerator<Company, Attribute> {
	...
	public void generateBlockingKeys(Company record, Processable<Correspondence<Attribute, Matchable>> correspondences,
		...
		if (record.getFounded() != null) {
			resultCollector.next(new Pair<>(Integer.toString(record.getFounded().getYear()), record));
		}
	}
