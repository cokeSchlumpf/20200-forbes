 ...
 *
 */
public class Release_IdentityResolution_Main_Discogs {
	/*
	 * Logging Options:
	...
	public static void main(String[] args) throws Exception {
		...
		xmlReader.loadFromXML(new File("usecase/music/mapforce/discogs_target_schema.xml"), "/releases/release",
				discogs);
		...
		gsTest.loadFromCSVFile(new File("usecase/music/goldstandard/musicbrainz_discogs_test.csv"));

		// Matching Rule
		// Linear Combination
		//TODO: adjust the parameter to define the threshold of the matching rule
		LinearCombinationMatchingRule<Release, Attribute> matchingRule = new LinearCombinationMatchingRule<Release, Attribute>(0.5);

		// activate logging
		// matchingRule.activateDebugReport("usecase/music/output/musicbrainz_2_discogs_log.csv", 100000, gsTest);
		...
		// matchingRule.addComparator(new ReleaseNameComparatorEqual(), 1.0);
		...
		// matchingRule.addComparator(new ReleaseArtistComparatorEqual(), 1.0);
		// matchingRule.addComparator(new ReleaseArtistComparatorJaccard(), 1.0);
		// matchingRule.addComparator(new ReleaseArtistComparatorLevenshtein(), 1.0);

		// matchingRule.addComparator(new ReleaseDateComparator2Years(), 1.0);
		// matchingRule.addComparator(new ReleaseDateComparator10Years(), 1.0);

		// matchingRule.addComparator(new ReleaseCountryComparatorEqual(), 1.0);
		// matchingRule.addComparator(new ReleaseCountryComparatorJaccard(), 1.0);
		// matchingRule.addComparator(new ReleaseCountryComparatorLevenshtein(), 1.0);

		// matchingRule.addComparator(new ReleaseDurationComparatorEqual(), 1.0);
		// matchingRule.addComparator(new ReleaseDurationComparator10Percent(), 1.0);

		// matchingRule.addComparator(new ReleaseTrackListComparatorSize(), 1.0);
