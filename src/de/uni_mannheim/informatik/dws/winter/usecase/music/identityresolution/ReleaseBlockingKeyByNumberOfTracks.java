 ...
 * 
 */
public class ReleaseBlockingKeyByNumberOfTracks extends RecordBlockingKeyGenerator<Release, Attribute> {
	...
	public void generateBlockingKeys(Release record, Processable<Correspondence<Attribute, Matchable>> correspondences,
		...
		if (record.getTracks() != null) {
			ret = record.getTracks().size();
		}
		resultCollector.next(new Pair<String, Release>(Double.toString(ret), record));
	}
