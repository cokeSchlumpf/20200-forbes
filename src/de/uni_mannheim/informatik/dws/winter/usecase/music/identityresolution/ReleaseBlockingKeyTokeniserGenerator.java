 ...
 * 
 */
public class ReleaseBlockingKeyTokeniserGenerator extends RecordBlockingKeyGenerator<Release, Attribute> {
	...
	public void generateBlockingKeys(Release record, Processable<Correspondence<Attribute, Matchable>> correspondences,
		...
		String text = String.format("%s %s %s %s", record.getName(), record.getArtist(), record.getCountry(), record.getLabel());
		String[] tokens = text.split("[^\\w']+");
		for(String s : tokens) {
			resultCollector.next(new Pair<String, Release>(s, record));
		}
	}
